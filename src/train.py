...
from data.dataloader import get_train_dataloader, get_valid_dataloader, ChampionTransform
...
def get_team_predictions(model, team_dataloader, encoder):
    ...
    predictions = []
    model.eval()
    for inputs, labels in team_dataloader:
...
def main(config_path):
    ...
    random_seed = config['dl']['random_seed']
    ...
    # get dataloaders
    train_dataloader, encoder = get_train_dataloader(champion_folder, background, output_size=65, object_size=(35, 45), batch_size=batch_size, shuffle=True)
    valid_dataloader, _ = get_valid_dataloader(encoder, data_path=game_img_path, team='right', batch_size=batch_size, shuffle=True)
    ...
    # train model
    model, val_hist = train_model(model, dataloaders, criterion, optimizer, num_epochs=num_epochs, device=device)

    # get team predictions
    left_dataloader, left_true_labels = get_valid_dataloader(encoder, data_path=game_img_path, team='left', batch_size=1, shuffle=False)
    right_dataloader, right_true_labels = get_valid_dataloader(encoder, data_path=game_img_path, team='right', batch_size=1, shuffle=False)
...
if __name__ == "__main__":
    ...
    PARSER.add_argument("-c", "--config", help="filepath to config json", default="./config.json")
