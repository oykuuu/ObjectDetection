...
class LoLChampionDetector(object):
    ...
    def __init__(self, game_img_path = '../screenshot.png', champion_folder = '../champions/', n_players=5):
        ...
        self.champion_paths = [os.path.join(champion_folder, ch_file) for ch_file in os.listdir(champion_folder)]
        self.champion_names = [ch_file.split('.')[0] for ch_file in os.listdir(champion_folder)]
    ...
    def find_best_scale(self, predictions):
        ...
        for key_name, _ in predictions.items():
            ...
            top_match_champions = [predictions[key_name][0][id] for id in top_match_index]
...
def main(config_path):
    ...
    left_players, ordered_champions, ordered_vals = lol_object.get_matched_players(left_predictions, best_size)
    right_players, ordered_champions, ordered_vals = lol_object.get_matched_players(right_predictions, best_size)
